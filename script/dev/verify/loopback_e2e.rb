...
section "Step 1 — Register work item MS-123 (kind: :jira)"

register = WorkCoordinator::Application::RegisterWorkItem.new(
  work_item_repo: repo,
  event_store: event_store
)
work_item = register.call(title: "Update Kafka fixture", kind: :jira, external_reference: "MS-123")

result = assert(work_item.external_reference == "MS-123", "work item MS-123 registered")
...
section "Step 4 — Assert human received [MS-123] prefixed message"

sent = msg_sender.sent_messages
expected_prefix = "[MS-123] #{question_body}"

result = assert(sent.length == 1, "exactly one message sent to human")
result == :pass ? passes += 1 : failures += 1

result = assert(sent.first[:body].start_with?(expected_prefix), "message starts with [MS-123] prefix")
result == :pass ? passes += 1 : failures += 1
...
raw_reply = "MS-123 yes, update the fixture and rerun the suite"
