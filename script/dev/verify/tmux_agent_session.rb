...
repo2 = FakeRepo.new({ "wi-2" => FakeWorkItem.new("wi-2", "growth-engine:claude.0") })
adapter2 = TestTmuxAgentSession.new(work_item_repo: repo2)
session_id = adapter2.start_session(work_item_id: "wi-2")
raise "expected 'growth-engine:claude.0', got #{session_id.inspect}" unless session_id == "growth-engine:claude.0"

pass "start_session returns tmux_target string as session_id"

section "deliver — calls correct tmux command"

adapter2.deliver(session_id: "growth-engine:claude.0", message: "hello world")
cmd = adapter2.commands.last
raise "expected tmux send-keys command" unless cmd[0..2] == ["tmux", "send-keys", "-t"]
raise "expected session target in command" unless cmd[3] == "growth-engine:claude.0"
...
repo4 = FakeRepo.new({ "wi-4" => FakeWorkItem.new("wi-4", "growth-engine:claude.0") })
adapter4 = TestTmuxAgentSession.new(work_item_repo: repo4)
adapter4.stub_command("list-panes", output: "growth-engine:claude.0\nother:1.0")

result = adapter4.active_session(work_item_id: "wi-4")
raise "expected 'growth-engine:claude.0', got #{result.inspect}" unless result == "growth-engine:claude.0"

pass "active_session returns target when pane is listed"

section "active_session — returns nil when pane missing"

repo5 = FakeRepo.new({ "wi-5" => FakeWorkItem.new("wi-5", "growth-engine:claude.0") })
...
ret = adapter2.end_session(session_id: "growth-engine:claude.0")
