...
RSpec.describe WorkCoordinator::Application::HandleQuery do
    ...
    context "when aliases are configured" do
      let(:config) do
        instance_double(WorkCoordinator::Config,
                        aliases: { "GE" => "growth-engine", "BI" => "billing-service" },
      ...
      it "returns the formatted alias list" do
        ...
        expect(result).to include("GE -> growth-engine")
        expect(result).to include("BI -> billing-service")
      end
    end
  ...
  describe "status <filter>" do
    before do
      repo.save(build(:work_item_domain,
                      external_reference: "GE-100",
                      title: "Active task",
                      state: :active))
      repo.save(build(:work_item_domain,
                      external_reference: "GE-200",
                      title: "Blocked task",
                      state: :blocked))
    end
    ...
    it "filters to active items" do
      ...
      expect(result).to include("GE-100")
      expect(result).not_to include("GE-200")
    end

    it "fuzzy-matches 'bloc' to blocked" do
      result = call("status bloc")
      expect(result).to include("GE-200")
      expect(result).not_to include("GE-100")
    end
  ...
  describe "shorthand status" do
    before do
      repo.save(build(:work_item_domain,
                      external_reference: "GE-300",
                      title: "Blocked item",
                      state: :blocked))
      repo.save(build(:work_item_domain,
                      external_reference: "GE-400",
                      title: "Waiting item",
                      state: :waiting_for_human))
    end
    ...
    it "'blocked' shorthand filters to blocked items" do
      result = call("blocked")
      expect(result).to include("GE-300")
      expect(result).not_to include("GE-400")
    end

    it "'waiting' maps to waiting_for_human" do
      result = call("waiting")
      expect(result).to include("GE-400")
      expect(result).not_to include("GE-300")
    end
  end
  ...
  describe "item <ref>" do
    context "when the item exists" do
      ...
      it "returns item detail", :aggregate_failures do
        ...
        expect(result).to include("GE-123: Fix login timeout")
        expect(result).to include("state: active")
        expect(result).to include("phase: implementing")
        expect(result).to include("repo: billing-service")
    ...
    context "when the item does not exist" do
      it "returns a not-found message" do
        result = call("item GE-999")
        expect(result).to include("No work item with ref: GE-999")
      end
    end
  end
  ...
  describe "non-matching body" do
    it "returns nil for something that looks like an AI action" do
      expect(call("claude billing-service fix the timeout")).to be_nil
    end
