...
RSpec.describe WorkCoordinator::Domain::AiCommand do
  ...
  context "with 'claude GE - add validation'" do
    ...
    it { expect(command.workspace).to eq("GE") }
    it { expect(command.instructions).to eq("add validation") }
  end

  context "with 'main growth-engine - refactor auth'" do
    let(:body) { "main growth-engine - refactor auth" }

    it { expect(command.verb).to eq("main") }
    it { expect(command.workspace).to eq("growth-engine") }
