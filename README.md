...
Messages arrive through two channels. **Local socket mode** listens on a Unix domain socket, so `work-coordinator send 'GE-123 go ahead'` from any terminal lands in the right pane. **Messages.app mode** polls `~/Library/Messages/chat.db` on macOS, so an agent can text you a question and you can answer from your iPhone. Both run at once by default.
